...
import torch
from util.batchnorm import SynchronizedBatchNorm2d

__all__ = ['ResNet', 'resnet18_aspp', 'resnet34_aspp', 'resnet50_aspp', 'resnet101_aspp',
           'resnet152_aspp']
...
class _ASPPModule(nn.Module):
    def __init__(self, inplanes, planes, kernel_size, padding, dilation, BatchNorm):
        ...
        self.atrous_conv = nn.Conv2d(inplanes, planes, kernel_size=kernel_size, groups=4,
                                            stride=1, padding=padding, dilation=dilation, bias=False)
...
class ASPP(nn.Module):
    def __init__(self, inplanes, output_stride, BatchNorm=nn.BatchNorm2d):
        super(ASPP, self).__init__()
        if output_stride == 16:
            dilations = [1, 6, 12, 18] #pixel-anchor [1, 3, 6, 9, 12, 15, 18]
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=7, scale=1):
        ...
        self.aspp = build_aspp(2048, 8, SynchronizedBatchNorm2d)
...
def resnet18_aspp(pretrained=False, **kwargs):
...
def resnet34_aspp(pretrained=False, **kwargs):
...
def resnet152_aspp(pretrained=False, **kwargs):
